...
local function check_and_load_waypoints()
    for _, tp in ipairs(enums.helltide_tps) do
        ...
        if match then
            ...
            tracker.current_maiden_position = enums.maiden_positions[tp.file] or nil
...
function explore_states:navigate_to_waypoint(target_index)
    ...
    if tracker.current_target_index == nil or tracker.current_target_index ~= target_index or tracker.dijkstra_path == nil then
        tracker.current_target_index = target_index
        local range_threshold = 35
        local path = explorerlite:dijkstra(waypoints, current_index, target_index, range_threshold)
        if not path then
            return false
        end

        tracker.dijkstra_path = path
        tracker.current_path_index = 1
        console.print(table.concat(path, " -> "))
    end

    local path = tracker.dijkstra_path
    local player_pos = tracker.player_position
    ...
    if player_pos:dist_to(current_wp) < 3 then
        if current_path_index == #path then
            --console.print("Target finale raggiunto.")
            return true
        else
...
local function should_activate_alfred()
...
explore_states.EXPLORE_HELLTIDE = {
    ...
    execute = function(sm)
        ...
        if should_activate_alfred() then
        ...
        if tracker.check_time("helltide_delay_find_chests", 1.6) then
            ...
            for chest_name, _ in pairs(enums.helltide_chests_info) do
                ...
                if chest_found and chest_found:is_interactable() and utils.distance_to(chest_found) < 35 then
            ...
            if gui.elements.open_silent_chests_toggle:get() then
                ...
                for _, item in pairs(player_consumable_items) do
                    if item:get_skin_name() == "GamblingCurrency_Key" then
                        ...
                        if silent_chest and silent_chest:is_interactable() and utils.distance_to(silent_chest) < 35 then
        ...
        --START EVENT MAIDEN
        local curent_maiden_hearts = get_helltide_coin_hearts()
        local attempts_maiden = gui.elements.maiden_slider_attempts:get() * 3
        local minutes_maiden = gui.elements.maiden_slider_minutes:get() * 60
        if tracker.check_time("helltide_delay_trigger_maiden", minutes_maiden) and curent_maiden_hearts >= attempts_maiden and gui.elements.maiden_enable_toggle:get() then
            tracker.attempts_maiden_track = gui.elements.maiden_slider_attempts:get()
            if gui.elements.maiden_return_to_origin_toggle:get() then
                tracker.last_position_waypoint_index =  tracker.waypoint_index
            end

...
gui.elements = {
    ...
    maiden_slider_attempts = create_slider_int(1, 100, 1, "maiden_slider_attempts"),
    maiden_slider_minutes = create_slider_int(1, 60, 20, "maiden_slider_minutes"),
...
function gui.draw_status(current_task)
    local messages = {
        ...
        { 
            text = "Next Trigger Maiden In : " .. current_task:get_next_maiden_msg(), 
            color = current_task:get_next_maiden_msg_color()
        },
        { 
...
function gui.render()
    if not gui.elements.main_tree:push("S Helltide | EmptyZero | " .. version) then return end
    ...
    if gui.elements.draw_status_settings_tree:push("Info / Status - Settings") then
        gui.elements.draw_status_offset_x:render("Offset X","")
        gui.elements.draw_status_offset_y:render("Offset Y","")
    ...
    if gui.elements.maiden_settings_tree:push("Maiden - Settings") then
        gui.elements.maiden_enable_toggle:render("Enable Maiden", "Ena")
        gui.elements.maiden_return_to_origin_toggle:render("Returns to point of origin", "")
        render_menu_header("Hearts needed to trigger maiden: " .. (gui.elements.maiden_slider_attempts:get() * 3) .. " Hearts")
        gui.elements.maiden_slider_attempts:render("Attempts", "Att")
        render_menu_header("Try to trigger maiden every: " .. gui.elements.maiden_slider_minutes:get() .. " Minutes")
        gui.elements.maiden_slider_minutes:render("Minutes", "")
        render_menu_header("So, try to maiden every "..gui.elements.maiden_slider_minutes:get().." minutes\nfor "..gui.elements.maiden_slider_attempts:get().." times ( only if he has enough hearts\nfor the attempts ("..(gui.elements.maiden_slider_attempts:get() * 3)..") )")
    ...
    if gui.elements.chests_settings_tree:push("Chests - Settings") then
        ...
        gui.elements.return_to_origin_toggle:render("Returns to point of origin", "Returns the player to the point of origin after attempting to open a previously visited chest")

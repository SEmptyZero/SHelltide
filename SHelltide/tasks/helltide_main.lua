...
local helltide_task = {
    ...
    get_next_maiden_msg = function(self)
        if gui.elements.maiden_enable_toggle:get() then
            return tracker.time_left("helltide_delay_trigger_maiden")
        end

        return ""
    end,

    get_next_maiden_msg_color = function(self)
        if not gui.elements.maiden_enable_toggle:get() or not tracker.helltide_delay_trigger_maiden then
            return color_white(255)
        end

        if get_helltide_coin_hearts() >= (3 * gui.elements.maiden_slider_attempts:get()) then
            return color_green(255)
        end
